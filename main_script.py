...
def generate_report():
    ...
    try:
        ...
        with pd.ExcelWriter("csgo_report.xlsx") as writer:
            if not final_matches.empty:
                final_matches.to_excel(writer, sheet_name="比赛汇总", index=False)
                print(f"已写入 {len(final_matches)} 条比赛汇总数据")
            else:
                print("警告：无比赛汇总数据")
                
            if not final_details.empty:
                final_details.to_excel(writer, sheet_name="玩家数据", index=False)
                print(f"已写入 {len(final_details)} 条玩家数据")
            else:
                print("警告：无玩家数据")
            
            if not final_infos.empty:
                final_infos.to_excel(writer, sheet_name="比赛详情", index=False)
                print(f"已写入 {len(final_infos)} 条比赛详情")
            else:
                print("警告：无比赛详情数据")
                
        print("报告生成完成！保存路径：csgo_report.xlsx")
    except Exception as e:
